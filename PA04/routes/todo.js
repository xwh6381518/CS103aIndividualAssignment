...
router.get('/todo/byUser',
  ...
  async (req, res, next) => {
    let results =
      await ToDoItem.aggregate( //aggregation
        [{
          $group: { //group by userId and count total
            _id: '$userId',
            total: { $count: {} }
          }
        },
        { $sort: { total: -1 } }, //sort in descending order by total
        ])

    results =
      await User.populate(results,
        {
          path: '_id', //replace the id by all information contained in id: id, username and age
          select: ['username', 'age']
        })
